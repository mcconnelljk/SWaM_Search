...
def report_option_menu(conn, products_df, vendor_id, products_dict):
    ...
    if selection == 'A' or selection == 'B':
        ...
        if len(results_list) > 0:
            ...
            report_option_menu(conn, products_df, vendor_id, products_dict)    
        else:
...
def get_vendor_orders(conn, products_df, vendor_id):
    products_dict = features.get_data.get_products_list(conn, vendor_id)
...
def validate_vendor_id(conn, products_df, vendor_id):
    vendors_list = features.get_data.get_vendors_list(conn)
    is_valid = vendor_id in vendors_list
    del(vendors_list)
    if len(vendor_id) > 0:
        if is_valid == True:
            if vendor_id.isdigit() == False:
            ...
            logic.errors.invalid_entry()
            input_vendor_id(conn, products_df)
    else:
        main_menu.print_main_menu(conn, products_df)

def input_vendor_id(conn, products_df):
    instructions = '\n Please input your unique, all-numeric Vendor ID, or press <<Return>> to go back...\n'
    print(instructions)
    selection = input('\n Vendor ID: ')
    vendor_id = validate_vendor_id(conn, products_df, selection)
    return(vendor_id)

def if_vendor(conn, products_df):
    vendor_id = input_vendor_id(conn, products_df)
    get_vendor_orders(conn, products_df, vendor_id)
    return